% run_simulations_residuals_global_local_shuf.m
% to generate a new set of 
% 540 x 100 simulations
% (# param combos x # iter)
% will test residuals, global shuffling, local shuffling
...
%%
randdir = pwd;
randfile = 'rnd_info_sim_res_shufG_shufL.mat';
...
nFO = numel(foArr);
nIter = 100;
fld1arr = {'compG','compL','res'}; % comp = shuffling compensated
nF1 = numel(fld1arr);
		...
		for pb = 1:nPB
			...
			for M = 1:nM
				...
				parfor i = 1:nIter % iteration loop
					...
					[sim_out,shuf_results_G,shuf_results_L,res_results] = run_sim_compare_res_global_local_shuf(simOpt);
