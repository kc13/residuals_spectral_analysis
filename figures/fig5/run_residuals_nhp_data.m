...
%% PSD / residuals preferences
psdopt.win_len_msec = boundSpec.endoffset*1000;
psdopt.min_oscil_spk = 0;
psdopt.fs_range = [0.1 100];
psdopt.store_input = false;
psdopt.store_stats = true;
psdopt.method = 'Welch';
psdopt.order = 'auto';
psdopt.oneModel = true;
psdopt.storeAllExpMdls = false;
psdopt.verbose = true;
psdopt.run_parfor = true;
...
for a = 1:nA  
	...
	for i = 1:nI
		...
		disp('running original and residuals-corrected psd')
        p_results(i) = interevent_residual_psd(src_spk_t,start_events,end_events,...
            boundSpec,psdopt);		
	end %i
