...
%% PSD / shuffling preferences
psdopt.n_shuf = 100;
psdopt.win_len_msec = boundSpec.endoffset*1000;
psdopt.min_oscil_spk = min_spk_win;
psdopt.fs_range = [0.1 100];
psdopt.store_shuf = false; 
psdopt.store_input = false;
psdopt.store_stats = true;
psdopt.method = 'Welch'; % not really used
psdopt.scope = 'windowedGlobal';
psdopt.verbose = true;
psdopt.filterFRs = true;
psdopt.storeGMMinfo = true;

%%
min_sec = 30; 
min_wins = ceil(min_sec*1000/psdopt.win_len_msec); 
...
for a = 1:nA  
    ...
    clear c_results p_results ev ev_clean
	for i = 1:nI
		...
		disp('running original and shuffling-corrected psd')
        p_results(i) = interevent_psd(src_spk_t,start_events,end_events,...
            boundSpec,psdopt);		
	end %i
	unit_data.(Area) = u_data;
    corr_results.(Area) = c_results;
    psd_results.(Area) = p_results;  
