%fig4_eval_synthetic.m
% set write preferences at end
%%
clear all;
close all;
clc;
...
gridfile = 'HRFAtable_stdFR_stdRP.mat';
gridpath = fullfile(datadir,gridfile);
rpfile = 'RPtable_stdFR_stdRP.mat';
...
oti = 'synthetic spike trains: method evaluation (see caption for hit/FA defintion; "hit" is FA if modulation = 0%)';
otisz = 10.5;
nP = 3; nC = 1; 
...
for g = 1:nG
	...
	nexttile(TL2)
	figopt.hlightCoords = {[40,41];[60,11]};
...
if testwrite
    outdir = pwd;
    outfile = 'fig4_eval_synthetic.tif';
    mkdir(outdir)
