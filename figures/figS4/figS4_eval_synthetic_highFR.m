...
gridfile = 'HRFAtable_highFR_stdRP.mat';
gridpath = fullfile(datadir,gridfile);
rpfile = 'RPtable_highFR_stdRP.mat';
...
oti = sprintf('synthetic spike trains: method evaluation (high FRs, %u ms RP, k = %0.1f)',R.simOpt.nr,R.simOpt.k);
otisz = 10.5;
nP = 3; nC = 1; 
...
tstr = sprintf('results sample #1: duration = 60 x 1024 ms, %u Hz oscillation, 100 simulation iterations per cell',f_osc);
...
% shift so make exponent visible
tshift = [0.48 +0.00005 0];
ti.Position = ti.Position + tshift;
%%
...
if testwrite
    outdir = pwd;
    outfile = 'figS4_eval_synthetic_highFR.tif';
    mkdir(outdir)
